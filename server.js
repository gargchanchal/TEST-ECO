...
// Fallback: map legacy .jpg/.jpeg image requests to .svg placeholders
app.get('/images/:name.jpg', (req, res) => {
	const { name } = req.params;
	return res.redirect(302, `/images/${name}.svg`);
});
app.get('/images/:name.jpeg', (req, res) => {
	const { name } = req.params;
	return res.redirect(302, `/images/${name}.svg`);
});
...
app.post('/checkout', async (req, res, next) => {
	try {
		if (!isStripeConfigured) {
			// Redirect back to cart with a friendly message when Stripe is not configured
